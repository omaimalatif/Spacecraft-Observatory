...
// One color per human-spaceflight family (all from CelesTrak's single
// GROUP=stations feed — see backend/app/routers/human_spaceflight.py) so the
// globe, sky plot and legend all agree. Every orbit line, LOS line and point
// marker is colored from this single table — nothing is hardcoded per-render.
export const CATEGORY_COLOR = {
  ISS: '#22c55e',
  'Tiangong / CSS': '#ef4444',
  'Crew & Cargo Vehicle': '#3b82f6',
  Other: '#7d8795',
}
...
export const CATEGORY_ORDER = ['ISS', 'Tiangong / CSS', 'Crew & Cargo Vehicle']
...
export default function HsfGlobe({ selected, onSelect, presets: presetsProp, location: locationProp, onLocationChange }) {
  ...
  // "global" = every crewed station/vehicle on the live catalog, no single location involved.
  ...
  // --- live human-spaceflight positions, polled on the same cadence as the backend cache ---
  useEffect(() => {
    let cancelled = false
    function poll() {
      api.humanSpaceflightGlobeObjects()
  ...
  useEffect(() => {
    ...
    api.humanSpaceflightOrbitPaths()
      .then((result) => {
        if (cancelled) return
        const byNorad = {}
  ...
  return (
    <div ref={wrapRef} className={`cesium-globe-wrap ${isFullscreen ? 'cesium-fullscreen' : ''}`}>
      ...
      <div className="cesium-overlay cesium-titlebar" style={{ top: 12, left: 12, right: 12 }}>
        <div>
          <p className="cesium-title">3D HUMAN SPACEFLIGHT VIEW</p>
          <p className="cesium-subtitle">
            {isLocal
              ...
              : 'All crewed stations & vehicles, global'}
          </p>
        </div>
      </div>
      ...
      {!data && !error && <div className="cesium-overlay cesium-status" style={{ top: 96 }}>Fetching live human-spaceflight catalog and propagating orbits (SGP4)…</div>}
