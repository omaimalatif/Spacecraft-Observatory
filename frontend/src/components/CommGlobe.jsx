...
// One color per communication satellite category (one real CelesTrak group
// each — see backend/app/routers/communication.py) so the globe, sky plot
...
export const CATEGORY_ORDER = ['Intelsat', 'SES', 'Eutelsat', 'Telesat', 'Iridium NEXT', 'Orbcomm', 'Globalstar', 'Amateur Radio']
...
export default function CommGlobe({ selected, onSelect, presets: presetsProp, location: locationProp, onLocationChange }) {
  ...
  // "global" = every communication satellite on the live catalog, no single location involved.
  ...
  // portal (lifted into CommunicationDashboard) so picking a place there — search, preset,
  ...
  // --- live communication-satellite positions, polled on the same cadence as the backend cache ---
  useEffect(() => {
    let cancelled = false
    function poll() {
      api.communicationGlobeObjects()
  ...
  return (
    <div ref={wrapRef} className={`cesium-globe-wrap ${isFullscreen ? 'cesium-fullscreen' : ''}`}>
        ...
        <div>
          ...
          <p className="cesium-subtitle">
            {isLocal
              ...
              : 'All communication satellites, global'}
          </p>
        </div>
      </div>
      ...
      {!data && !error && <div className="cesium-overlay cesium-status" style={{ top: 96 }}>Fetching live communication-satellite catalog and propagating orbits (SGP4)…</div>}
