...
// One color per GNSS constellation, matching the reference sky-plot palette
// (GPS green / GLONASS red / Galileo blue / BeiDou amber / QZSS yellow /
// SBAS purple / NavIC teal) so the globe, sky plot and legend all agree.
// Every orbit line, LOS line and point marker is colored from this single
// table — nothing is hardcoded per-render.
...
export const CONSTELLATION_ORDER = ['GPS', 'GLONASS', 'Galileo', 'BeiDou', 'QZSS', 'SBAS', 'NavIC']
...
export default function NavGlobe({ selected, onSelect, presets: presetsProp, location: locationProp, onLocationChange }) {
  ...
  const [constellationFilter, setConstellationFilter] = useState(
    Object.fromEntries(Object.keys(CONSTELLATION_COLOR).map((c) => [c, true]))
  )
  ...
  // "global" = every GNSS satellite on the live catalog, no single location involved.
  ...
  // portal (lifted into NavigationDashboard) so picking a place there — search, preset,
  ...
  // --- live GNSS positions, polled on the same cadence as the backend cache ---
  useEffect(() => {
    let cancelled = false
    function poll() {
      api.navigationGlobeObjects()
  ...
  const globalVisibleObjects = useMemo(() => {
    ...
    return data.objects.filter((o) => constellationFilter[o.constellation ?? 'Other'])
  }, [data, constellationFilter])
  ...
  const constellationCounts = useMemo(() => {
    const counts = {}
    for (const o of data?.objects ?? []) counts[o.constellation ?? 'Other'] = (counts[o.constellation ?? 'Other'] ?? 0) + 1
    return counts
  }, [data])
  ...
  useEffect(() => {
    ...
    visibleObjects.forEach((obj, i) => {
      ...
      const color = cesiumColor(CONSTELLATION_COLOR[obj.constellation] ?? CONSTELLATION_COLOR.Other)
  ...
  function toggleConstellation(c) {
  ...
  return (
    <div ref={wrapRef} className={`cesium-globe-wrap ${isFullscreen ? 'cesium-fullscreen' : ''}`}>
      ...
      <div className="cesium-overlay cesium-titlebar" style={{ top: 12, left: 12, right: 12 }}>
        <div>
          <p className="cesium-title">3D CONSTELLATION VIEW</p>
          <p className="cesium-subtitle">
            {isLocal
              ...
              : 'All GNSS constellations, global'}
          </p>
        </div>
      </div>
      ...
      {data && (
        <div className="cesium-overlay cesium-controls" style={{ top: 56, left: 12 }}>
          ...
          <div className="cesium-chip-row">
            {[...CONSTELLATION_ORDER, 'Other'].filter((c) => constellationCounts[c] > 0).map((c) => (
              <button
                key={c}
                className={`legend regime-chip constellation-chip ${constellationFilter[c] ? '' : 'off'}`}
                style={{ borderColor: CONSTELLATION_COLOR[c], color: constellationFilter[c] ? CONSTELLATION_COLOR[c] : undefined }}
                onClick={() => toggleConstellation(c)}
              >
                {c} ({fmt(constellationCounts[c])})
              </button>
            ))}
          </div>
      ...
      {!data && !error && <div className="cesium-overlay cesium-status" style={{ top: 96 }}>Fetching live GNSS catalog and propagating orbits (SGP4)…</div>}
