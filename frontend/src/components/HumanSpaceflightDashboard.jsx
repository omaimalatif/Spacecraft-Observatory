...
function AvailabilityPanel({ onSelect, presets, location, onLocationChange }) {
  ...
  useEffect(() => {
    api.humanSpaceflightGlobeObjects().then((data) => setGlobalObjects(data.objects || [])).catch(() => setGlobalObjects([]))
  }, [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    api.humanSpaceflightAvailability({ lat: location.lat, lon: location.lon, minElevation: mask })
  ...
  return (
    <div className="availability-stage">
      <article className="panel availability-map-panel">
          ...
          {loading && !availability && <p className="loading-hint">Propagating human-spaceflight orbits for this location (SGP4)…</p>}
      ...
      <article className="panel availability-results" id="availability">
          ...
          <h2>Crewed stations & vehicles above the horizon</h2>
          {!availability ? (
...
function CrewPanel({ crew, issPosition }) {
  if (!crew && !issPosition) return null
  return (
    <section className="panel crew-panel">
      <p className="eyebrow">WHO'S IN SPACE RIGHT NOW</p>
      <h2>{crew ? `${crew.number} people currently off-planet` : 'Loading crew roster…'}</h2>
      {issPosition && (
        <p className="crew-iss-note">
          ISS is currently over {num(issPosition.iss_position?.latitude, 2)}°, {num(issPosition.iss_position?.longitude, 2)}°
        </p>
      )}
      {crew && (
        <div className="crew-grid">
          {crew.people.map((p) => (
            <div className="crew-chip" key={p.name}>
              <b>{p.name}</b>
              <span>{p.craft}</span>
            </div>
          ))}
      ...
      <p className="crew-source">Source: Open Notify — not a CelesTrak / SGP4 feed.</p>
    </section>
  )
}
...
export default function HumanSpaceflightDashboard() {
  ...
  useEffect(() => {
    api.peopleInSpace().then(setCrew).catch(() => setCrew(null))
    api.issNow().then(setIssPosition).catch(() => setIssPosition(null))
  }, [])

  useEffect(() => {
    api.humanSpaceflightOverview()
      .then(setOverview)
      .catch((err) => setError(err.message || 'Could not load live human-spaceflight catalog'))
    api.humanSpaceflightServiceInfo().then(setServiceInfo).catch(() => setServiceInfo(null))
  ...
  async function submitSearch(e) {
    ...
    setSearching(true)
    try {
      const r = await api.humanSpaceflightSatellites({ q: query })
    ...
    <main className="assets-page hsf-page">
      ...
      <section className="assets-intro">
        <div>
          ...
          <h1>Human <em>Spaceflight</em></h1>
          <p>Track the crewed stations and vehicles that keep people living and working in space. Follow the ISS and China's Tiangong (CSS) live, along with any crew or cargo vehicle currently docked or in transit — all from CelesTrak's single official space-stations group — inspect their orbital parameters, and check which are visible from any location.</p>
        </div>
      ...
      <CrewPanel crew={crew} issPosition={issPosition} />

      <section className="command-grid">
        <article className="globe-card panel">
          <div className="panel-head">
            <div><p className="eyebrow">HUMAN SPACEFLIGHT 3D ORBITS · CESIUM</p><h2>Crewed stations & vehicles around Earth</h2></div>
      ...
      <section id="breakdown" className="snapshot">
        <div className="section-title">
          ...
          <span>Classified human-spaceflight inventory</span>
        </div>
        <div className="snapshot-grid">
          ...
          <article id="sources" className="panel search-card nav-search-card">
            <p className="eyebrow">HUMAN SPACEFLIGHT SEARCH</p>
            <h2>Locate a station, module, or crew/cargo vehicle</h2>
      ...
      <footer className="assets-footer">
        ...
        Crew &amp; position data: <a href="http://open-notify.org/" target="_blank" rel="noreferrer">Open Notify</a> ·
        Reference specs: NASA and China Manned Space Agency official sites ·
