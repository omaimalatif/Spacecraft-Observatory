...
import SciGlobe, { CATEGORY_COLOR, CATEGORY_ORDER } from './SciGlobe.jsx'
import SciSkyPlot from './SciSkyPlot.jsx'
import LocationSearch from './LocationSearch.jsx'
import SciLocationPicker from './SciLocationPicker.jsx'
...
function AvailabilityPanel({ onSelect, presets, location, onLocationChange }) {
  ...
  useEffect(() => {
    api.spaceScienceSatGlobeObjects().then((data) => setGlobalObjects(data.objects || [])).catch(() => setGlobalObjects([]))
  }, [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    api.spaceScienceSatAvailability({ lat: location.lat, lon: location.lon, minElevation: mask })
  ...
  return (
    <div className="availability-stage">
      <article className="panel availability-map-panel">
          ...
          <SciLocationPicker lat={location?.lat} lon={location?.lon} onPick={setLocation} satellites={mapSatellites} />
          {location && !location.label && (
            <p className="loading-hint">Selected point: {location.lat.toFixed(3)}°, {location.lon.toFixed(3)}°</p>
          )}
          {loading && !availability && <p className="loading-hint">Propagating space-science-satellite orbits for this location (SGP4)…</p>}
      ...
      <article className="panel availability-results" id="availability">
          ...
          <h2>Space science satellites above the horizon</h2>
          {!availability ? (
...
function ServiceInfoGrid({ serviceInfo }) {
  if (!serviceInfo) return <p className="loading-hint">Loading mission-family reference data…</p>
  return (
...
export default function SpaceScienceSatellites() {
  ...
  useEffect(() => {
    api.spaceScienceSatOverview()
      .then(setOverview)
      .catch((err) => setError(err.message || 'Could not load live space-science-satellite catalog'))
    api.spaceScienceSatServiceInfo().then(setServiceInfo).catch(() => setServiceInfo(null))
  ...
  async function submitSearch(e) {
    ...
    setSearching(true)
    try {
      const r = await api.spaceScienceSatSatellites({ q: query })
  ...
  return (
    <main className="assets-page scisat-page">
      <header className="assets-header">
        ...
        <nav>
          ...
          <PortalMenu />
          <a className="active">06 Live Space Science Tracking</a>
        </nav>
      ...
      <section className="assets-intro">
        <div>
          <p className="eyebrow">PORTAL 06 · SATELLITES</p>
          <h1>Space Science <em>Satellite Tracking</em></h1>
          <p>Meet the Earth-orbiting telescopes, science platforms, and technology-demonstration satellites that CelesTrak tracks under its Space &amp; Earth Science group. Track live astrophysics, heliophysics, Earth &amp; climate science, and tech-demo missions, inspect their orbital parameters, follow their positions around Earth, and check which are visible from any location. Deep-space probes (JWST, Voyager, Parker Solar Probe) live in the Solar System tab above instead — this view only covers Earth-orbiting science satellites with trackable orbital elements.</p>
        </div>
      ...
      <section className="command-grid">
        <article className="globe-card panel">
          <div className="panel-head">
            <div><p className="eyebrow">SCIENCE SATELLITE 3D ORBITS · CESIUM</p><h2>Space science satellites around Earth</h2></div>
      ...
      <section id="breakdown" className="snapshot">
        <div className="section-title">
          <p className="eyebrow">MISSION FAMILY BREAKDOWN</p>
          <h2>Satellites per science mission family</h2>
          <span>Classified space-science satellite inventory</span>
        </div>
        <div className="snapshot-grid">
          ...
          <article id="sources" className="panel search-card nav-search-card">
            <p className="eyebrow">SPACE SCIENCE SATELLITE SEARCH</p>
            <h2>Locate a space-science satellite</h2>
      ...
      <section id="service-info" className="activity-grid" style={{ gridTemplateColumns: '1fr' }}>
        <article className="panel activity" style={{ gridColumn: '1 / -1' }}>
          <p className="eyebrow">MISSION FAMILY CONTEXT</p>
          <h2>Space &amp; Earth science mission families</h2>
      ...
      <footer className="assets-footer">
        Data attribution: <a href="https://celestrak.org/" target="_blank" rel="noreferrer">CelesTrak GP data</a> (GROUP=science) ·
        Reference context: <a href="https://science.nasa.gov/" target="_blank" rel="noreferrer">NASA Science</a>, <a href="https://www.nasa.gov/mission/small-spacecraft-technology-program/" target="_blank" rel="noreferrer">NASA Small Spacecraft Technology</a> ·
        Mission-family groupings are best-effort, derived from public satellite names — see the "Other / Unclassified" category for anything not confidently matched. Live data is cached server-side for up to 5 minutes.
      </footer>
    </main>
  )
}
