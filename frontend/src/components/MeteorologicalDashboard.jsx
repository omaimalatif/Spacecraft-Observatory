...
import MeteoGlobe, { CATEGORY_COLOR, CATEGORY_ORDER } from './MeteoGlobe.jsx'
import MeteoSkyPlot from './MeteoSkyPlot.jsx'
import LocationSearch from './LocationSearch.jsx'
import MeteoLocationPicker from './MeteoLocationPicker.jsx'
...
function AvailabilityPanel({ onSelect, presets, location, onLocationChange }) {
  ...
  useEffect(() => {
    api.meteorologicalGlobeObjects().then((data) => setGlobalObjects(data.objects || [])).catch(() => setGlobalObjects([]))
  }, [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    setError(null)
    api.meteorologicalAvailability({ lat: location.lat, lon: location.lon, minElevation: mask })
  ...
  return (
    <div className="availability-stage">
      <article className="panel availability-map-panel">
          ...
          <MeteoLocationPicker lat={location?.lat} lon={location?.lon} onPick={setLocation} satellites={mapSatellites} />
          {location && !location.label && (
            <p className="loading-hint">Selected point: {location.lat.toFixed(3)}°, {location.lon.toFixed(3)}°</p>
          )}
          {loading && !availability && <p className="loading-hint">Propagating meteorological-satellite orbits for this location (SGP4)…</p>}
      ...
      <article className="panel availability-results" id="availability">
          ...
          <h2>Meteorological satellites above the horizon</h2>
          {!availability ? (
...
export default function MeteorologicalDashboard() {
  ...
  useEffect(() => {
    api.meteorologicalOverview()
      .then(setOverview)
      .catch((err) => setError(err.message || 'Could not load live meteorological-satellite catalog'))
    api.meteorologicalServiceInfo().then(setServiceInfo).catch(() => setServiceInfo(null))
  ...
  async function submitSearch(e) {
    ...
    setSearching(true)
    try {
      const r = await api.meteorologicalSatellites({ q: query })
  ...
  return (
    <main className="assets-page meteo-page">
      <header className="assets-header">
        ...
        <nav>
          ...
          <PortalMenu />
          <a className="active">05 Meteorological</a>
        </nav>
      ...
      <section className="assets-intro">
        <div>
          <p className="eyebrow">PORTAL 05</p>
          <h1>Meteorological <em>&amp; Environmental Satellites</em></h1>
          <p>Follow the missions that forecast the weather and monitor the environment. Track live GOES, Meteosat/MTG, Metop, NOAA POES/JPSS, Fengyun, DMSP, and Roscosmos (Meteor-M/Elektro-L/Arktika-M) satellites — all from CelesTrak's single official weather group — inspect their orbital parameters, follow their positions around Earth, and check which are visible from any location.</p>
        </div>
      ...
      <section className="command-grid">
        <article className="globe-card panel">
          <div className="panel-head">
            <div><p className="eyebrow">SATELLITE FAMILY 3D ORBITS · CESIUM</p><h2>Meteorological satellites around Earth</h2></div>
      ...
      <section id="breakdown" className="snapshot">
        <div className="section-title">
          ...
          <h2>Satellites per meteorological family</h2>
          <span>Classified meteorological satellite inventory</span>
        </div>
        <div className="snapshot-grid">
          ...
          <article id="sources" className="panel search-card nav-search-card">
            <p className="eyebrow">METEOROLOGICAL SATELLITE SEARCH</p>
            <h2>Locate a meteorological satellite</h2>
      ...
      <footer className="assets-footer">
        Data attribution: <a href="https://celestrak.org/" target="_blank" rel="noreferrer">CelesTrak GP data</a> (GROUP=weather; family labels derived by name-pattern within that group) ·
        Reference specs: NOAA NESDIS, EUMETSAT, China Meteorological Administration, U.S. Space Force / NOAA and Roshydromet/Roscosmos official sites ·
