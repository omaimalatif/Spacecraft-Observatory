...
// One color per meteorological satellite family (all from CelesTrak's single
// GROUP=weather feed — see backend/app/routers/meteorological.py) so the globe,
// sky plot
...
export const CATEGORY_COLOR = {
  ...
  'Meteosat / MTG': '#ef4444',
  Metop: '#3b82f6',
  'NOAA POES / JPSS': '#d97706',
  Fengyun: '#eab308',
  DMSP: '#a855f7',
  'Roscosmos (Meteor/Elektro/Arktika)': '#14b8a6',
  Other: '#7d8795',
}
...
export const CATEGORY_ORDER = ['GOES', 'Meteosat / MTG', 'Metop', 'NOAA POES / JPSS', 'Fengyun', 'DMSP', 'Roscosmos (Meteor/Elektro/Arktika)']
...
export default function MeteoGlobe({ selected, onSelect, presets: presetsProp, location: locationProp, onLocationChange }) {
  ...
  // "global" = every meteorological satellite on the live catalog, no single location involved.
  ...
  // portal (lifted into MeteorologicalDashboard) so picking a place there — search, preset,
  ...
  // --- live meteorological-satellite positions, polled on the same cadence as the backend cache ---
  useEffect(() => {
    let cancelled = false
    function poll() {
      api.meteorologicalGlobeObjects()
  ...
  return (
    <div ref={wrapRef} className={`cesium-globe-wrap ${isFullscreen ? 'cesium-fullscreen' : ''}`}>
        ...
        <div>
          ...
          <p className="cesium-subtitle">
            {isLocal
              ...
              : 'All meteorological satellites, global'}
          </p>
        </div>
      </div>
      ...
      {!data && !error && <div className="cesium-overlay cesium-status" style={{ top: 96 }}>Fetching live meteorological-satellite catalog and propagating orbits (SGP4)…</div>}
