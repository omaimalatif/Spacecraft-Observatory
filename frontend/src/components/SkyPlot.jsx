...
import { CONSTELLATION_COLOR, CONSTELLATION_ORDER } from './NavGlobe.jsx'
  ...
  useEffect(() => {
    ...
    for (const satellite of satellites || []) {
      ...
      const point = new THREE.Mesh(
        ...
        new THREE.MeshBasicMaterial({ color: CONSTELLATION_COLOR[satellite.constellation] ?? CONSTELLATION_COLOR.Other })
      )
    ...
    if (showOrbits && tracks) {
      for (const satellite of satellites || []) {
        ...
        const color = CONSTELLATION_COLOR[satellite.constellation] ?? CONSTELLATION_COLOR.Other
        let segment = []
        const flushSegment = () => {
          if (segment.length > 1) {
            const line = new THREE.Line(
...
export default function SkyPlot({ satellites, mask, onMaskChange, catalogSize, location, onSelect }) {
  const [mode, setMode] = useState('2D')
  const [constellationFilter, setConstellationFilter] = useState(
    Object.fromEntries([...CONSTELLATION_ORDER, 'Other'].map((c) => [c, true]))
  )
  ...
  const visibleSats = useMemo(
    () => (satellites || []).filter((s) => constellationFilter[s.constellation ?? 'Other']),
  ...
  function handleReset() {
    setConstellationFilter(Object.fromEntries([...CONSTELLATION_ORDER, 'Other'].map((c) => [c, true])))
    setShowOrbits(false)
    setShowLabels(true)
    setShowLOS(false)
    setMode('2D')
  ...
  useEffect(() => {
    ...
    api.navigationSkyTracks({ lat: location.lat, lon: location.lon, minElevation: mask, windowMin: 25 })
  ...
  const trackPolylines2D = useMemo(() => {
    ...
    return Object.entries(tracks).map(([noradId, entry]) => {
      ...
      if (current.length > 1) segments.push(current)
      return { noradId, constellation: entry.constellation, segments }
    })
  ...
  return (
    <article className={`panel skyplot-panel ${fullscreen ? 'skyplot-fullscreen' : ''}`}>
      ...
      <div className="skyplot-toolbar">
        {[...CONSTELLATION_ORDER, 'Other'].filter((c) => (satellites || []).some((s) => (s.constellation ?? 'Other') === c) || constellationFilter[c] === false).map((c) => (
          <button
            key={c}
            className={`constellation-chip ${constellationFilter[c] ? '' : 'off'}`}
      ...
      >
        ...
        {showOrbits && trackPolylines2D.map(({ noradId, constellation, segments }) => (
          <g key={noradId}>
            {segments.map((seg, i) => (
              <polyline
                key={i}
                ...
                style={{ stroke: CONSTELLATION_COLOR[constellation] ?? CONSTELLATION_COLOR.Other }}
              />
            ))}
          </g>
        ))}
        ...
        {visibleSats.map((sat) => {
          ...
          const color = CONSTELLATION_COLOR[sat.constellation] ?? CONSTELLATION_COLOR.Other
