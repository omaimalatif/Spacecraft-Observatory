...
// One color per space-science mission family (best-effort, name-derived —
// see backend/app/routers/sci_satellites.py) so the globe, sky plot and
// legend all agree. Every orbit line, LOS line and point marker is colored
// from this single table — nothing is hardcoded per-render.
export const CATEGORY_COLOR = {
  'Astrophysics & Astronomy': '#a855f7',
  'Heliophysics & Space Weather': '#eab308',
  'Earth & Climate Science': '#22c55e',
  'Technology Demonstration': '#3b82f6',
  'Other / Unclassified': '#7d8795',
}
...
export const CATEGORY_ORDER = ['Astrophysics & Astronomy', 'Heliophysics & Space Weather', 'Earth & Climate Science', 'Technology Demonstration']
...
export default function SciGlobe({ selected, onSelect, presets: presetsProp, location: locationProp, onLocationChange }) {
  ...
  // "global" = every space-science satellite on the live catalog, no single location involved.
  ...
  // portal (lifted into the Space Science dashboard) so picking a place there — search, preset,
  ...
  // --- live space-science-satellite positions, polled on the same cadence as the backend cache ---
  useEffect(() => {
    let cancelled = false
    function poll() {
      api.spaceScienceSatGlobeObjects()
  ...
  return (
    <div ref={wrapRef} className={`cesium-globe-wrap ${isFullscreen ? 'cesium-fullscreen' : ''}`}>
      ...
      <div className="cesium-overlay cesium-titlebar" style={{ top: 12, left: 12, right: 12 }}>
        <div>
          <p className="cesium-title">3D SCIENCE SATELLITE VIEW</p>
          <p className="cesium-subtitle">
            {isLocal
              ...
              : 'All space-science satellites, global'}
          </p>
        </div>
      </div>
      ...
      {!data && !error && <div className="cesium-overlay cesium-status" style={{ top: 96 }}>Fetching live space-science-satellite catalog and propagating orbits (SGP4)…</div>}
